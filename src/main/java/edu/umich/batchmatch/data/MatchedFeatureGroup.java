...
public class MatchedFeatureGroup {
	...
	public MatchedFeatureGroup(int key, List<FeatureFromFile> features, Boolean handleCorrs) {
		...
		for (int i = 0; i < features.size(); i++) {
			// if (features.get(i).getBatchIdx() == 10)
			/// System.out.println("Added 10 to group " + matchGrpKey);

			sb.append(features.get(i).getBatchIdx() + "_");
		}

		// System.out.println("Size of group is " + (features == null ? 0 :
		// features.size()));
	...
	public int countPIs() {
		...
		String annotation;
		for (FeatureFromFile f : this.getFeaturesInGroup()) {
			annotation = f.getAnnotation();
			if (!StringUtils.isEmptyOrNull(annotation)) {
				String[] tokens = StringUtils.splitAndTrim(annotation, "+");
	...
	public void ensureHighestCorrs() {
		ensureHighestCorrs(false);
	}

	public void ensureHighestCorrs(Boolean allowIncomplete) {
	...
	public Map<String, Map<Integer, Double>> buildCompoundBatchCorrMap() {
	...
	public void parseMatchEntry(String stringToParse, Map<String, Map<Integer, Double>> compoundBatchCorrMap) {
	...
	public Double updateTotalIntensity() {
		totalIntensity = null;
	...
	public Double updateAvgMedIntensity() {
		avgMedIntensity = null;
		return getAvgMedIntensity();
	}

	public Double updateAvgRt() {
		avgRt = null;
		return getAvgRt();
	}
	...
	public Double updateAvgOldRt() {
		avgOldRt = null;
		return getAvgOldRt();
	}

	public void addFeature(FeatureFromFile f) {
		this.featuresInGroup.add(f);
		updateStats();
	}

	public void addFeatures(List<FeatureFromFile> lst) {
	...
	public String printMatchInfo2() {

		updateStats();

		Double rt1 = featuresInGroup.get(0).getOldRt();
		Double rt2 = featuresInGroup.get(1).getOldRt();

		Double massDiff = getMassDiff();
		Double oldRtDiff = getOldRtDiff();

		String summaryFormat = "%5.4f %5.4f %5.4f %5.4f";
		String output = String.format(summaryFormat, rt1, rt2, massDiff, oldRtDiff);
		return output;
	}

	public String printMatchInfo() {

		updateStats();

		Double mass1 = featuresInGroup.get(0).getMass();
		Double mass2 = featuresInGroup.get(1).getMass();

		Double massDiff = getMassDiff();
		Double oldRtDiff = getOldRtDiff();

		String summaryFormat = "%5.4f %5.4f %5.4f %5.4f";
		String output = String.format(summaryFormat, mass1, mass2, massDiff, oldRtDiff);
		return output;
	}

//.006
	public String printOldRts(Boolean forCSV) {
		updateStats();

		String summaryFormat = "%5.4f %5.4f %5.4f";
		if (forCSV)
			summaryFormat = "%5.4f,%5.4f,%5.4f";

		Double oldRt1 = featuresInGroup.get(0).getOldRt();
		Double oldRt2 = featuresInGroup.get(1).getOldRt();

		String output = String.format(summaryFormat, oldRt1, oldRt2, getOldRtDiff());
		return output;
	}
	...
	public void printFeatureGroup() {

		Collections.sort(featuresInGroup, new FeatureByRtAndMassComparator());

		for (int i = 0; i < featuresInGroup.size(); i++)
			System.out.println(featuresInGroup.get(i).toCSVString2());
	 ...
	 * grabMatchedNamedFeatureGroupsOfSize(int nBatches, Boolean includeAmbiguous) {
	 * Map<Integer, MatchedFeatureGroup> namedMatchGroups =
	 * grabMatchedNamedFeatureGroups(); Map<Integer, MatchedFeatureGroup>
	 * namedMatchGroupsOfSize = new HashMap<Integer, MatchedFeatureGroup>();
	 * 
	 * for (MatchedFeatureGroup grp : namedMatchGroups.values()) { if
	 * (grp.getBatches().size() == nBatches) if (includeAmbiguous ||
	 * grp.getFeatureNames().size() == nBatches)
	 * namedMatchGroupsOfSize.put(grp.getMatchGrpKey(), grp); } return
	 * namedMatchGroupsOfSize; }
	 */
	 ...
	 * 
	 * Map<Integer, MatchedFeatureGroup> namedMatchGroups = new HashMap<Integer,
	 * MatchedFeatureGroup>(); for (MatchedFeatureGroup grp :
	 * matchedFeatureGroups.values()) { if (grp.countMatchedTargetFeatures(null) >
	 * 0) namedMatchGroups.put(grp.getMatchGrpKey(), grp); } return
	 * namedMatchGroups; }
	 */
...
/*
 * Integer matchGrpKey; String batchesStr = ""; List<FeatureFromFile>
 * featuresInGroup;
 * 
 * private Double avgMass = null; private Double avgRt = null, avgOldRt = null;
 * private Double massDiff = null, oldRtDiff = null, rtDiff = null;
 * 
 * 
 * public MatchedFeatureGroup(int key, List<FeatureFromFile> features) {
 * this.matchGrpKey = key; this.featuresInGroup = features; StringBuilder sb =
 * new StringBuilder(); Collections.sort(features, new
 * FeatureByBatchComparator()); for (int i = 0; i < features.size(); i++) { //if
 * (features.get(i).getBatchIdx() == 10) ///
 * System.out.println("Added 10 to group " + matchGrpKey);
 * 
 * sb.append(features.get(i).getBatchIdx() + "_"); }
 * 
 * //System.out.println("Size of group is " + (features == null ? 0 :
 * features.size())); batchesStr = sb.toString();
 * 
 * updateStats(); }
 * 
 * public MatchedFeatureGroup(MatchedFeatureGroup grp1, MatchedFeatureGroup
 * grp2) { matchGrpKey = grp1.getMatchGrpKey(); featuresInGroup = new
 * ArrayList<FeatureFromFile>(); for (FeatureFromFile f :
 * grp2.getFeaturesInGroup())
 * f.setFurtherAnnotation((StringUtils.isEmptyOrNull(f.getFurtherAnnotation()) ?
 * "" : f.getFurtherAnnotation() + "; ") + " Merged from match group " +
 * grp2.getMatchGrpKey() + " ");
 * 
 * //Signature is addFeatures(grp1.getFeaturesInGroup());
 * addFeatures(grp2.getFeaturesInGroup());
 * 
 * for (FeatureFromFile f : featuresInGroup) f.setRedundancyGroup(matchGrpKey);
 * updateStats(); }
 * 
 * public List<String> getFeatureNames() { List<String> featureNames = new
 * ArrayList<String>();
 * 
 * for (int i = 0; i < featuresInGroup.size(); i++) {
 * featureNames.add(featuresInGroup.get(i).getName()); } return featureNames; }
 * 
 * public Double getAvgMass() {
 * 
 * if (avgMass == null) { avgMass = 0.0; for (FeatureFromFile f :
 * featuresInGroup ) avgMass += f.getMass();
 * 
 * avgMass /= featuresInGroup.size(); }
 * 
 * return avgMass; }
 * 
 * public Double updateAvgMass() { avgMass = null; return getAvgMass(); }
 * 
 * public List<Integer> getBatches() {
 * 
 * List<Integer> list = new ArrayList<Integer>(); for (FeatureFromFile f :
 * featuresInGroup) { list.add(f.getBatchIdx()); } return list; }
 * 
 * 
 * 
 * public String getBatchesStr() { return batchesStr; }
 * 
 * public void setBatchesStr(String batchesStr) { this.batchesStr = batchesStr;
 * }
 * 
 * public Double getAvgRt() {
 * 
 * if (avgRt == null) { avgRt = 0.0; for (FeatureFromFile f : featuresInGroup )
 * avgRt += f.getRt();
 * 
 * avgRt /= featuresInGroup.size(); }
 * 
 * return avgRt; }
 * 
 * public Double updateAvgRt() { avgRt = null; return getAvgRt(); }
 * 
 * public Double getAvgOldRt() {
 * 
 * if (avgOldRt == null) { avgOldRt = 0.0; for (FeatureFromFile f :
 * featuresInGroup ) avgOldRt += f.getOldRt() == null ? f.getRt() :
 * f.getOldRt();
 * 
 * avgOldRt /= featuresInGroup.size(); } return avgOldRt; }
 * 
 * public Double updateAvgOldRt() { avgOldRt = null; return getAvgOldRt(); }
 * 
 * public void addFeature(FeatureFromFile f) { this.featuresInGroup.add(f);
 * updateStats(); }
 * 
 * public void addFeatures(List<FeatureFromFile> lst) {
 * 
 * for (FeatureFromFile f : lst) featuresInGroup.add(f); updateStats(); }
 * 
 * public void updateStats() { updateAvgOldRt(); updateAvgRt(); updateAvgMass();
 * updatePairStats(); }
 * 
 * private void updatePairStats() { if (featuresInGroup.size() == 2) { massDiff
 * = featuresInGroup.get(1).getMass() - featuresInGroup.get(0).getMass(); if
 * (featuresInGroup.get(1).getOldRt() != null &&
 * featuresInGroup.get(0).getOldRt() != null) oldRtDiff =
 * featuresInGroup.get(1).getOldRt() - featuresInGroup.get(0).getOldRt();
 * 
 * rtDiff = featuresInGroup.get(1).getRt() - featuresInGroup.get(0).getRt(); } }
 * 
 * public void clearFeatures() { this.featuresInGroup.clear(); updateStats(); }
 * 
 * public Integer getMatchGrpKey() { return matchGrpKey; }
 * 
 * public void setMatchGrpKey(Integer matchGrpKey) { this.matchGrpKey =
 * matchGrpKey; }
 * 
 * public List<FeatureFromFile> getFeaturesInGroup() { return featuresInGroup; }
 * 
 * public void setFeaturesInGroup(List<FeatureFromFile> featuresInGroup) {
 * this.featuresInGroup = featuresInGroup; }
 * 
 ...
 * 
 * public void setAvgRt(Double avgRt) { this.avgRt = avgRt; }
 * 
 * public void setAvgOldRt(Double avgOldRt) { this.avgOldRt = avgOldRt; }
 * 
 * public Double getMassDiff() { updatePairStats(); return massDiff; }
 * 
 * 
 * public Double getOldRtDiff() { updatePairStats(); return oldRtDiff; }
 * 
 * public Double getRtDiff() { updatePairStats(); return rtDiff; }
 * 
 * public String printMatchInfo2() {
 * 
 * updateStats();
 * 
 * 
 * Double rt1 = featuresInGroup.get(0).getOldRt(); Double rt2 =
 * featuresInGroup.get(1).getOldRt();
 * 
 * Double massDiff = getMassDiff(); Double oldRtDiff = getOldRtDiff();
 * 
 * String summaryFormat = "%5.4f %5.4f %5.4f %5.4f"; String output =
 * String.format(summaryFormat, rt1, rt2, massDiff, oldRtDiff); return output; }
 * 
 * public Integer countMatchedTargetFeatures(Map<String, String> targetFeatures)
 * {
 * 
 * Integer count = 0; if (targetFeatures != null) { for (FeatureFromFile f :
 * featuresInGroup) { if (targetFeatures.containsKey(f.getName())) count++; } }
 * else { for (FeatureFromFile f : featuresInGroup) { if
 * (!f.getName().startsWith("UNK_")) count++; } } return count; }
 * 
 * 
 * public String printMatchInfo() {
 * 
 ...
 * 
 * 
 * Double mass1 = featuresInGroup.get(0).getMass(); Double mass2 =
 * featuresInGroup.get(1).getMass();
 * 
 * Double massDiff = getMassDiff(); Double oldRtDiff = getOldRtDiff();
 * 
 * String summaryFormat = "%5.4f %5.4f %5.4f %5.4f"; String output =
 * String.format(summaryFormat, mass1, mass2, massDiff, oldRtDiff); return
 * output; } public String printOldRts(Boolean forCSV) { updateStats();
 * 
 * String summaryFormat = "%5.4f %5.4f %5.4f"; if (forCSV) summaryFormat =
 * "%5.4f,%5.4f,%5.4f";
 * 
 * Double oldRt1 = featuresInGroup.get(0).getOldRt(); Double oldRt2 =
 * featuresInGroup.get(1).getOldRt();
 * 
 * String output = String.format(summaryFormat, oldRt1, oldRt2, getOldRtDiff());
 * return output; }
