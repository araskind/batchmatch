...
import edu.umich.batchmatch.data.comparators.FeatureByBatchComparator;
import edu.umich.batchmatch.data.comparators.FeatureByMassAndRtComparator;
import edu.umich.batchmatch.data.comparators.FeatureByNameComparator;
import edu.umich.batchmatch.process.AnnealingEngine2;
...
public class PostProcessDataSet {
	...
	public Boolean correctDuplicateFeatureNames() {
		int iters = 1;
		Boolean featuresUnique = false;
		while (!featuresUnique) {
			featuresUnique = detectObviousDuplicateFeatures();
			iters++;
			if (iters > 4) {
				System.out.println("Warning couldn't correct all duplicate feature names after 4 tries");
	...
	private Boolean detectObviousDuplicateFeatures() {
		...
		// }
		Collections.sort(features, new FeatureByNameComparator());

		List<Integer> duplicateIndices = new ArrayList<Integer>();

		for (int i = 1; i < features.size(); i++) {
			if (features.get(i).getName().equals(features.get(i - 1).getName())) {
				duplicateIndices.add(i);
				System.out.println("Warning duplicate feature name " + features.get(i).getName());
			}
		}

		Integer nConsecutiveDups = 1;

		for (int i = 1; i < duplicateIndices.size(); i++) {
			// if ((Math.abs(duplicateIndices.get(i) - duplicateIndices.get(i - 1)) > 0)) {
			// nConsecutiveDups = 1;
			// }
			// else {
			// nConsecutiveDups++;
			...
			String newName1 = features.get(i).getName() + "_" + features.get(i).getMass() + "-"
					+ features.get(i).getBatchIdx();
			// String newName2 = features.get(i-1).getName() + "-" +
			// (features.get(i-1).getMedianIntensity() == null ? "null" :
			// features.get(i-1).getMedianIntensity()) + "-" +
			// features.get(i-1).getBatchIdx() + "-" + features.get(i-1).getMass();
			String newName2 = features.get(i - 1).getName() + "_" + features.get(i - 1).getMass() + "-"
					+ features.get(i - 1).getBatchIdx();

			features.get(i - 1).setName(newName2);
			features.get(i).setName(newName1); // oldName + "-" + features.get(i).getMass() + "-" +
												// features.get(i).getBatchIdx());
		}
		...
		for (int i = 1; i < features.size(); i++) {
			if (features.get(i).getName().equals(features.get(i - 1).getName())) {
				System.out.println("Warning: persistent duplicate feature name " + features.get(i).getName());
				// return false;
			}
		}
	...
	public Map<Integer, List<FeatureFromFile>> createFeatureByRedundancyGroupMap() {
	...
	public Map<String, Double> buildMatchGroupToStdDevRtMap() {

		Map<String, List<Double>> matchGroupToFeatureRtsMap = new HashMap<String, List<Double>>();
		...
		for (FeatureFromFile feature : getFeatures()) {
			...
			name = feature.getRedundancyGroup().toString();
			if (!matchGroupToFeatureRtsMap.containsKey(name))
				matchGroupToFeatureRtsMap.put(name, new ArrayList<Double>());

			matchGroupToFeatureRtsMap.get(name).add(feature.getRT());
		}

		StandardDeviation std = new StandardDeviation();
		Map<String, Double> matchGrpToStdDevMap = new HashMap<String, Double>();
		for (String matchGrp : matchGroupToFeatureRtsMap.keySet()) {
			List<Double> values = matchGroupToFeatureRtsMap.get(matchGrp);

			double[] nativeValues = new double[values.size()];

			for (int i = 0; i < values.size(); i++)
				nativeValues[i] = values.get(i);

			matchGrpToStdDevMap.put(matchGrp, std.evaluate(nativeValues));
		}
		return matchGrpToStdDevMap;
	}
	...
	public void identifyMatchedFeatures(Integer annealingStep, double stretchFactor, double massTol, double rtTol,
		...
		// Screen existing map
		// pair diverging
		Boolean useAverageForFeatureMatching = true;
...
//Candidate lattice points
	/*
	 * private void identifyMatchedFeaturesByRange(Map<Integer,
	 * List<FeatureFromFile>> redundancyMap, int maxRedundancyGrp, double massTol,
	 * double rtTol, Boolean combineGroups) {
	 * 
	 * FeatureFromFile feature1, feature2; int nRedundancyGroups = maxRedundancyGrp
	 * + 1; Integer grp1, grp2; double min, max, diff1, diff2, diffRt, diffMass; for
	 * (int i = 0; i < features.size() - 1; i++) { feature1 = features.get(i);
	 * 
	 * int upper = features.size() - i; for (int j = 1; j < upper; j++) {
	 * 
	 * feature2 = features.get(i + j);
	 * 
	 * if (feature1.getMass() == null || feature1.getRT() == null) continue;
	 * 
	 * if (feature2.getMass() == null || feature2.getRT() == null) continue;
	 * 
	 * grp1 = feature1.getRedundancyGroup(); grp2 = feature2.getRedundancyGroup();
	 * 
	 * if (feature2.getBatchIdx().equals(feature1.getBatchIdx())) continue;
	 * 
	 * if (!(feature1.getPutativeCharge() == null) && !(feature2.getPutativeCharge()
	 * == null) &&
	 * !feature1.getPutativeCharge().equals(feature2.getPutativeCharge())) continue;
	 * 
	 * if (grp1 != null && grp2 != null) { min =
	 * Math.min(minRtsByMatchGrp.get(grp2), minRtsByMatchGrp.get(grp1)); max =
	 * Math.max(maxRtsByMatchGrp.get(grp2), maxRtsByMatchGrp.get(grp1)); diffRt =
	 * Math.abs(max - min);
	 * 
	 * min = Math.min(minMassesByMatchGrp.get(grp2), minMassesByMatchGrp.get(grp1));
	 * max = Math.max(maxMassesByMatchGrp.get(grp2), maxMassesByMatchGrp.get(grp1));
	 * diffMass = Math.abs(max - min); } else if (grp1 == null && !(grp2 == null)){
	 * diff1 = feature1.getRt() - minRtsByMatchGrp.get(grp2); diff2 =
	 * feature1.getRt() - maxRtsByMatchGrp.get(grp2); diffRt =
	 * Math.max(Math.abs(diff1), Math.abs(diff2));
	 * 
	 * diff1 = feature1.getMass() - minMassesByMatchGrp.get(grp2); diff2 =
	 * feature1.getMass() - maxMassesByMatchGrp.get(grp2); diffMass =
	 * Math.max(Math.abs(diff1), Math.abs(diff2)); } else if (grp2 == null && !(grp1
	 * == null)) { diff1 = feature2.getRt() - minRtsByMatchGrp.get(grp1); diff2 =
	 * feature2.getRt() - maxRtsByMatchGrp.get(grp1); diffRt =
	 * Math.max(Math.abs(diff1), Math.abs(diff2));
	 * 
	 * diff1 = feature2.getMass() - minMassesByMatchGrp.get(grp1); diff2 =
	 * feature2.getMass() - maxMassesByMatchGrp.get(grp1); diffMass =
	 * Math.max(Math.abs(diff1), Math.abs(diff2)); } else { diffRt =
	 * Math.abs(feature1.getRt() - feature2.getRt()); diffMass=
	 * Math.abs(feature1.getMass() - feature2.getMass()); }
	 * 
	 * if (Math.abs(diffRt) > rtTol || Math.abs(diffMass) > massTol) continue;
	 * 
	 * if (!redundantFeatureMap.containsKey(feature1.getName()))
	 * redundantFeatureMap.put(feature1.getName(), new ArrayList<String>());
	 * 
	 * redundantFeatureMap.get(feature1.getName()).add(feature2.getName());
	 * 
	 * feature1.setFlaggedAsDuplicate(true); feature2.setFlaggedAsDuplicate(true);
	 * 
	 * Boolean newGroup = (feature1.getRedundancyGroup() == null &&
	 * feature2.getRedundancyGroup() == null);
	 * 
	 * if (newGroup) { feature1.setRedundancyGroup(nRedundancyGroups);
	 * feature2.setRedundancyGroup(nRedundancyGroups);
	 * 
	 * grpSizeByMatchGrp.put(nRedundancyGroups, 2);
	 * avgRtsByMatchGrp.put(nRedundancyGroups, 0.5 * (feature1.getRt() +
	 * feature2.getRt())); avgMassesByMatchGrp.put(nRedundancyGroups, 0.5 *
	 * (feature1.getMass() + feature2.getMass()));
	 * 
	 * minRtsByMatchGrp.put(nRedundancyGroups, Math.min(feature1.getRt(),
	 * feature2.getRt())); minMassesByMatchGrp.put(nRedundancyGroups,
	 * Math.min(feature1.getMass(), feature2.getMass()));
	 * 
	 * maxRtsByMatchGrp.put(nRedundancyGroups, Math.max(feature1.getRt(),
	 * feature2.getRt())); maxMassesByMatchGrp.put(nRedundancyGroups,
	 * Math.max(feature1.getMass(), feature2.getMass()));
	 * 
	 * if (!redundancyMap.containsKey(nRedundancyGroups))
	 * redundancyMap.put(nRedundancyGroups, new ArrayList<FeatureFromFile>());
	 * 
	 * redundancyMap.get(nRedundancyGroups).add(feature1);
	 * redundancyMap.get(nRedundancyGroups).add(feature2);
	 * 
	 * nRedundancyGroups++; } else if (feature2.getRedundancyGroup() == null) { int
	 * grp = feature1.getRedundancyGroup(); feature2.setRedundancyGroup(grp); int
	 * oldSize = this.grpSizeByMatchGrp.get(grp); double oldMassAvg =
	 * this.avgMassesByMatchGrp.get(grp); double oldRtAvg =
	 * this.avgRtsByMatchGrp.get(grp); double newMassAvg = oldMassAvg * oldSize +
	 * feature2.getMass(); double newRtAvg = oldRtAvg * oldSize + feature2.getRt();
	 * oldSize++; newMassAvg /= (1.0 * oldSize); newRtAvg /= (1.0 * oldSize);
	 * 
	 * this.grpSizeByMatchGrp.put(grp, oldSize); this.avgMassesByMatchGrp.put(grp,
	 * newMassAvg); this.avgRtsByMatchGrp.put(grp, newRtAvg);
	 * 
	 * double featureMass = feature2.getMass(); double featureRt = feature2.getRt();
	 * 
	 * double oldMassMin = this.minMassesByMatchGrp.get(grp); double oldRtMin =
	 * this.minRtsByMatchGrp.get(grp);
	 * 
	 * this.minMassesByMatchGrp.put(grp, Math.min(featureMass, oldMassMin));
	 * this.minRtsByMatchGrp.put(grp, Math.min(featureRt, oldRtMin));
	 * 
	 * double oldMassMax = this.maxMassesByMatchGrp.get(grp); double oldRtMax =
	 * this.maxRtsByMatchGrp.get(grp);
	 * 
	 * this.maxMassesByMatchGrp.put(grp, Math.max(featureMass, oldMassMax));
	 * this.maxRtsByMatchGrp.put(grp, Math.max(featureRt, oldRtMax));
	 * 
	 * if (!redundancyMap.containsKey(grp)) redundancyMap.put(grp, new
	 * ArrayList<FeatureFromFile>());
	 * 
	 * redundancyMap.get(grp).add(feature2); } else if
	 * (feature1.getRedundancyGroup() == null) {
	 * 
	 * int grp = feature2.getRedundancyGroup(); feature1.setRedundancyGroup(grp);
	 * int oldSize = this.grpSizeByMatchGrp.get(grp); double oldMassAvg =
	 * this.avgMassesByMatchGrp.get(grp); double oldRtAvg =
	 * this.avgRtsByMatchGrp.get(grp); double newMassAvg = oldMassAvg * oldSize +
	 * feature1.getMass(); double newRtAvg = oldRtAvg * oldSize + feature1.getRt();
	 * oldSize++; newMassAvg /= (1.0 * oldSize); newRtAvg /= (1.0 * oldSize);
	 * 
	 * this.grpSizeByMatchGrp.put(grp, oldSize); this.avgMassesByMatchGrp.put(grp,
	 * newMassAvg); this.avgRtsByMatchGrp.put(grp, newRtAvg);
	 * 
	 * double featureMass = feature1.getMass(); double featureRt = feature1.getRt();
	 * 
	 * double oldMassMin = this.minMassesByMatchGrp.get(grp); double oldRtMin =
	 * this.minRtsByMatchGrp.get(grp);
	 * 
	 ...
	 * ArrayList<FeatureFromFile>());
	 * 
	 * redundancyMap.get(grp).add(feature1); } else if (grp1 != null && grp2 != null
	 * && combineGroups && redundancyMap != null) {
	 * 
	 * if (grp1.equals(grp2)) continue;
	 * 
	 * Integer grpToKeep = grp1 > grp2 ? grp2 : grp1 ; Integer grpToMerge = grp1 >
	 * grp2 ? grp1 : grp2;
	 * 
	 * int oldSize = this.grpSizeByMatchGrp.get(grpToKeep); double oldMassAvg =
	 * this.avgMassesByMatchGrp.get(grpToKeep); double oldRtAvg =
	 * this.avgRtsByMatchGrp.get(grpToKeep);
	 * 
	 * int mergedSize = this.grpSizeByMatchGrp.get(grpToMerge); double mergedMassAvg
	 * = this.avgMassesByMatchGrp.get(grpToMerge); double mergedRtAvg =
	 * this.avgRtsByMatchGrp.get(grpToMerge);
	 * 
	 * double newMassAvg = oldMassAvg * oldSize + mergedMassAvg * mergedSize; double
	 * newRtAvg = oldRtAvg * oldSize + mergedRtAvg * mergedSize; newMassAvg /= (1.0
	 * * (oldSize + mergedSize)); newRtAvg /= (1.0 * (oldSize + mergedSize));
	 * 
	 * this.grpSizeByMatchGrp.put(grpToKeep, oldSize + mergedSize);
	 * this.avgMassesByMatchGrp.put(grpToKeep, newMassAvg);
	 * this.avgRtsByMatchGrp.put(grpToKeep, newRtAvg);
	 * 
	 * if
	 * (minRtsByMatchGrp.get(grpToMerge).compareTo(minRtsByMatchGrp.get(grpToKeep))
	 * < 0) minRtsByMatchGrp.put(grpToKeep, minRtsByMatchGrp.get(grpToMerge));
	 * 
	 * if
	 * (maxRtsByMatchGrp.get(grpToMerge).compareTo(maxRtsByMatchGrp.get(grpToKeep))
	 * > 0) maxRtsByMatchGrp.put(grpToKeep, maxRtsByMatchGrp.get(grpToMerge));
	 * 
	 * if (minMassesByMatchGrp.get(grpToMerge).compareTo(minMassesByMatchGrp.get(
	 * grpToKeep)) < 0) minMassesByMatchGrp.put(grpToKeep,
	 * minMassesByMatchGrp.get(grpToMerge));
	 * 
	 * if (maxMassesByMatchGrp.get(grpToMerge).compareTo(maxMassesByMatchGrp.get(
	 * grpToKeep)) > 0) maxMassesByMatchGrp.put(grpToKeep,
	 * maxMassesByMatchGrp.get(grpToMerge));
	 * 
	 * if (redundancyMap.containsKey(grpToMerge)) { for (FeatureFromFile feature :
	 * redundancyMap.get(grpToMerge)) { feature.setRedundancyGroup(grpToKeep);
	 * feature.setAnnotation("Merged from " + grpToMerge + ";" +
	 * feature.getAnnotation()); redundancyMap.get(grpToKeep).add(feature); }
	 * redundancyMap.remove(grpToMerge); } } } } System.out.println("done"); }
	 */
	// Map<Double
	public Map<String, String> getDerivedNameToHeaderMap() {
		return derivedNameToHeaderMap;
		// if (derivedNameToHeaderMap != null)
		// return derivedNameToHeaderMap;
		// getDerivedSampleNames();
		// return derivedNameToHeaderMap;
	}
	...
	public void initializeRSDsForCollapsedFeatures() {
		...
		for (FeatureFromFile feature : features) {
			...
			/*
			 * nMissing = 0; for (String hdr : orderedIntensityHeaders) { if
			 * (StringUtils.isEmptyOrNull(hdr)) continue; if
			 * (StringUtils.isEmptyOrNull(feature.getValueForIntensityHeader(hdr,
			 * derivedNameToHeaderMap))) nMissing++; } feature.setPctMissing((100.0 *
			 * nMissing / (1.0 * nValuesPossible)));
			 */
		}
	...
	private void calculateRSDsFromSampleList(List<String> colsForRSD, Integer batchNo) {
		...
		if (colsForRSD != null && colsForRSD.size() < 1) {
			for (FeatureFromFile feature : features)
				feature.setRsd(null);
			// JOptionPane.showMessageDialog(null, "No controls have been specified -- RSD
			// values will not be calculated " +
			// PostProccessConstants.LINE_SEPARATOR + "any samples. ");
		}
...
/*
 * public void initializeRSDs(List<String> colsForRSD) {
 * 
 * List<String> nonSampleCols =
 * PostProcessMergeNameExtractor.determineNonSampleCols(orderedIntensityHeaders)
 * ;
 * 
 * if (colsForRSD == null) colsForRSD =
 * PostProcessMergeNameExtractor.determineNonSampleCols(orderedIntensityHeaders)
 * ;
 * 
 * List<String> nonStatisticCols = new ArrayList<String>(); for (String hdr :
 * orderedIntensityHeaders) if (colsForRSD != null) { if
 * (!StringUtils.isEmptyOrNull(hdr) && nonSampleCols.contains(hdr) &&
 * !colsForRSD.contains(hdr)) nonStatisticCols.add(hdr); } int nColsToIgnore =
 * nonStatisticCols.size();
 * 
 * String featureColVal = null; int nMissing = 0; int nSampleCols =
 * orderedIntensityHeaders.size() - nColsToIgnore - colsForRSD.size();
 * 
 * for (FeatureFromFile feature : features) { nMissing = 0; for (String hdr :
 * orderedIntensityHeaders) { if (StringUtils.isEmptyOrNull(hdr) ||
 * nonSampleCols.contains(hdr) || colsForRSD.contains(hdr)) continue;
 * 
 * featureColVal = feature.getValueForIntensityHeader(hdr ,
 * derivedNameToHeaderMap); if (StringUtils.isEmptyOrNull(featureColVal) ||
 * "-".equals(featureColVal) || ".".equals(featureColVal)) { nMissing++; }
 * feature.setPctMissing(((100.0 * nMissing)/ (1.0 * nSampleCols))); } }
 * calculateRSDs(colsForRSD); }
 */
