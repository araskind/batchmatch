...
import edu.umich.med.mrc2.batchmatch.data.orig.BatchMatchFeatureInfo;
...
public class BatchMatchLatticeBuilder {
	...
	public Boolean buildLatticeFile(
		...
		for (int batchIndex = 0; batchIndex < BatchMatchConstants.N_BATCHES; batchIndex++) {
			...
			List<BatchMatchFeatureInfo> featuresForBatch = 
					getFeaturesByIntensity(
							textFile, 
							batchIndex, 
							mpColIndices,
							-1, 
		...
		for (int batch1 = 0; batch1 < BatchMatchConstants.N_BATCHES - 1; batch1++) {
			...
			for (int batch2 = batch1 + 1; batch2 < BatchMatchConstants.N_BATCHES; batch2++) {
				...
				outputFile = writeLatticeFile(
						...
						batch2, 
						batch1Label, 
						batch2Label,																								// batchTags.get(batch2),
						featuresByBatch, 
						outputDirectory, 
						rtToUse, 
						poolSampleSize);
	...
	private List<BatchMatchFeatureInfo> getFeaturesByIntensity(
			TextFile textFile, 
			int batch, 
			int[] mpColIndices,
			int binnerNameColIndex, 
			int rtExpectedColIndex, 
			int rtObservedColIndex, 
			int neutralMassColIdx) {
	...
	private List<BatchMatchFeatureInfo> grabTopAndSort(
			List<BatchMatchFeatureInfo> features, 
			Integer nPoolSamples) {
