...
import edu.umich.med.mrc2.batchmatch.io.sheetreaders.MetabolomicsTargetedDataLoader;
...
import edu.umich.med.mrc2.batchmatch.process.orig.BatchMatchLatticeBuilder;
import edu.umich.med.mrc2.batchmatch.utils.orig.ListUtils;

public class BatchMatchNamedUnnamedComparisonTabPanel extends StickySettingsPanel {
	...
	private FileListLoaderDisplayPanel binnerFileListLoaderPanel;
	private FileListLoaderDisplayPanel rawFileListLoaderPanel;
	private FileListLoaderDisplayPanel namedFileListLoaderPanel;
	private BatchMatchLatticeTypePanel latticeTypePanel;
	private JButton createLatticeSetButton;
	private JPanel batchLatticeWrapPanel;
	private JPanel createLatticeSetWrapPanel;
	private JPanel progPanel;
	private JProgressBar progBar;
	private String fileFileListName = "dd_lattice_list.csv";

	public BatchMatchNamedUnnamedComparisonTabPanel(SharedAnalysisSettings sharedAnalysisSettings) {
		super();
	...
	public void setupPanel(SharedAnalysisSettings sharedAnalysisSettings) {
		...
		outputDirectoryPanelBinnerData = new AbstractStickyFileLocationPanel(
				"Specify Output Directory ", "binnerizedfilelist.directory") {
		...
		binnerFileListLoaderPanel = new FileListLoaderDisplayPanel() {
			...
			protected void updateInterfaceForNewData(int chosen, ArrayList<String> fileNames) {
				...
				binnerFileListLoaderPanel.setInitialDirectoryForChooser(
						outputDirectoryPanelBinnerData.getOutputDirectoryPath());
		...
		rawFileListLoaderPanel.setupPanel("Select Raw Feature Data Files  (Drag and drop to map filename to batch)",
				"Update Labels", outputDirectoryPanel.getOutputDirectoryPath(), false, "Create Lattice Files");
		...
		namedFileListLoaderPanel.setupPanel("Select Profinder Files (Drag and drop to map filename to batch)",
				"Update Labels", outputDirectoryPanel.getOutputDirectoryPath(), false, "Create Lattice Files");
		...
		batchLatticeWrapPanel.add(Box.createVerticalStrut(1));
		// batchLatticeWrapPanel.add(poolSampleSizePanel);

		// batchLatticeWrapPanel.add(Box.createVerticalStrut(1));
		// batchLatticeWrapPanel.add(createLatticeSetWrapPanel);
		...
		add(Box.createVerticalStrut(1));
		LayoutUtils.addBlankLines(this, 45);
	}
	...
	private Boolean createNamedMassMap() {
		...
		for (String fileName : latticeMassAverages.keySet()) {
			...
			// targetLabel, true);
			completeLatticeFileNames.put(currIdx, latticeFile.getAbsolutePath());
		}
		writeFileFile(outputDirectory, completeLatticeFileNames);
	...
	private void writeFileFile(
			...
			Map<String, String> completeLatticeFileNames) {
	...
	private Boolean createLatticeSet() {
		...
		for (int i = 0; i < keyStrings.size(); i++) {
			...
			if (targetedFile.equals(targetedFile))
				continue;

			BatchMatchLatticeBuilder batchMatch = new BatchMatchLatticeBuilder();

			dataWritten = batchMatch.buildLatticeFile(
					batchFile, 
					targetedFile, 
					outputDirectory, 
					key,
			...
			outputFileNames.add(batchMatch.getOutputFile().getAbsolutePath());
			fileTags.add(key);
		}
	...
	private void printBinnerFileMap(
			String fileName, 
			Map<String, String> batchFileNameMap,
			Map<String, String> batchFileOrderMap, 
			boolean useBinnerDir) {
